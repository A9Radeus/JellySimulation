...
using namespace DirectX;
 ...
 namespace rch {
 XMFLOAT3 toXMF3_objl(objl::Vector3 v) { return XMFLOAT3(v.X, v.Y, v.Z); };
}  // namespace rch

Mesh Mesh::LoadObj(const DxDevice& device, const std::string& fn) {
    objl::Loader objLoader;
    if (objLoader.LoadFile(fn) == false) {
      throw "Couldn't load given .obj";
    }

    //std::vector<VertexPositionNormal> verts;

    auto objVerts = objLoader.LoadedVertices;
  
    std::vector<VertexPositionNormal> verts;
    verts.reserve(objVerts.size());
    for (const auto& vert : objVerts) {
      verts.push_back(
          {rch::toXMF3_objl(vert.Position), rch::toXMF3_objl(vert.Normal)});
    }

    auto objInds = objLoader.LoadedIndices;

    std::vector<unsigned short> inds;
    inds.reserve(objInds.size());
    for (const auto& idx : objInds) {
      inds.emplace_back(idx);
    }
...
std::pair<std::vector<VertexPositionNormal>, std::vector<unsigned short>>
Mesh::LoadObjBuffs(const DxDevice& device, const std::string& fn) {
  objl::Loader objLoader;
  if (objLoader.LoadFile(fn) == false) {
    throw "Couldn't load given .obj";
  }
...
std::vector<VertexPosition> mini::Mesh::BoxVerts(float width,
                                                      float height,
                                                      float depth) {
  return {// Front Face
...
// Counter-Clockwise (when looking from the OY+ towards OY-
std::vector<VertexPosition> mini::Mesh::BoxVertsUnique(float width,
                                                      float height,
                                                      float depth) {
  return {
          // Bottom Face
...
// @rch TODO
std::vector<VertexPositionNormal> 
mini::Mesh::ShadedBoxVertsCorner(float width, float height, float depth) {
  return {
      // Front face
      {{0, 0, 0}, {0.0f, 0.0f, -1.0f}},
      {{width, 0, 0}, {0.0f, 0.0f, -1.0f}},
      {{width, height, 0}, {0.0f, 0.0f, -1.0f}},
      {{0, height, 0}, {0.0f, 0.0f, -1.0f}},

      // Back face
      {{width, 0, depth}, {0.0f, 0.0f, 1.0f}},
      {{0, 0, depth}, {0.0f, 0.0f, 1.0f}},
      {{0, height, depth}, {0.0f, 0.0f, 1.0f}},
      {{width, height, depth}, {0.0f, 0.0f, 1.0f}},

      // Left face
      {{0, 0, depth}, {-1.0f, 0.0f, 0.0f}},
      {{0, 0, 0}, {-1.0f, 0.0f, 0.0f}},
      {{0, height, 0}, {-1.0f, 0.0f, 0.0f}},
      {{0, height, depth}, {-1.0f, 0.0f, 0.0f}},

      // Right face
      {{width, 0, 0}, {1.0f, 0.0f, 0.0f}},
      {{width, 0, depth}, {1.0f, 0.0f, 0.0f}},
      {{width, height, depth}, {1.0f, 0.0f, 0.0f}},
      {{width, height, 0}, {1.0f, 0.0f, 0.0f}},

      // Bottom face
      {{0, 0, depth}, {0.0f, -1.0f, 0.0f}},
      {{width, 0, depth}, {0.0f, -1.0f, 0.0f}},
      {{width, 0, 0}, {0.0f, -1.0f, 0.0f}},
      {{0, 0, 0}, {0.0f, -1.0f, 0.0f}},

      // Top face
      {{0, height, 0}, {0.0f, 1.0f, 0.0f}},
      {{width, height, 0}, {0.0f, 1.0f, 0.0f}},
      {{width, height, depth}, {0.0f, 1.0f, 0.0f}},
      {{0, height, depth}, {0.0f, 1.0f, 0.0f}},
  };
}
