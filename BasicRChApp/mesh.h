...
namespace mini
{
	...
	public:
		...
		static Mesh SimpleTriMesh(const DxDevice& device, const std::vector<VertexType> verts, const std::vector<unsigned short> idxs)
		{
			...
			result.m_indexCount = static_cast<unsigned int>(idxs.size());
    ...
    static Mesh AdjacencyTriMesh(const DxDevice& device,
      ...
      result.m_primitiveType =
		...
		//Box Mesh Creation
    static std::vector<VertexPosition> BoxVerts(float width, float height, float depth);
    static std::vector<VertexPosition> BoxVertsUnique(float width, float height, float depth);
    ...
    static std::vector<VertexPositionNormal> ShadedBoxVertsCorner(
        float width, float height, float depth);
		static std::vector<unsigned short> BoxIdxs();
		static Mesh Box(const DxDevice& device, float width, float height, float depth) { return SimpleTriMesh(device, BoxVerts(width, height, depth), BoxIdxs()); }
		static Mesh Cube(const DxDevice& device, float side) { return SimpleTriMesh(device, BoxVerts(side, side, side), BoxIdxs()); }
    ...
    static Mesh LoadDuck(const DxDevice& device, const std::wstring& meshPath);
		...
		static Mesh LoadObj(const DxDevice& device, const std::string& fn);

    static std::pair<std::vector<VertexPositionNormal>,
									   std::vector<unsigned short>>
    LoadObjBuffs(const DxDevice& device, const std::string& fn);
