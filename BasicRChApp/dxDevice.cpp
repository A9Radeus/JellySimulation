...
dx_ptr<ID3D11HullShader> mini::DxDevice::CreateHullShader(
    std::vector<BYTE> hsCode) const {
  ID3D11HullShader* hs = nullptr;
  auto hr =
      m_device->CreateHullShader(hsCode.data(), hsCode.size(), nullptr, &hs);
  dx_ptr<ID3D11HullShader> hullShader(hs);
  if (FAILED(hr)) THROW_DX(hr);
  return hullShader;
}

dx_ptr<ID3D11DomainShader> mini::DxDevice::CreateDomainShader(
    std::vector<BYTE> dsCode) const {
  ID3D11DomainShader* ds = nullptr;
  auto hr =
      m_device->CreateDomainShader(dsCode.data(), dsCode.size(), nullptr, &ds);
