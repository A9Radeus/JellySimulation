...
namespace mini
{
    ...
    dx_ptr<ID3D11DomainShader> CreateDomainShader(std::vector<BYTE> dsCode) const;
		...
		/*
				@R
		*/
    // @R Oryginalnie jest w dxApplication, ale bardziej pasuje
    // tutaj
    //		(przenoszone na potrzeby RenderingSystem)
    void updateBuffer(const dx_ptr<ID3D11Buffer>& buffer,
                      const void* data, size_t count) const {
      D3D11_MAPPED_SUBRESOURCE res;
      auto hr = m_context->Map(buffer.get(), 0,
                                D3D11_MAP_WRITE_DISCARD, 0, &res);
      //if (FAILED(hr)) {
      //  THROW_DX(hr);
      //}
      if (FAILED(hr)) {
				throw "[DxDevice] updateBuffer failed.";
			}
    ...
    // @R Wraper. Oryginalnie tak samo jak w glownym updateBuffer
    template <typename T>
    void updateBuffer(const dx_ptr<ID3D11Buffer>& buffer,
                      const T& data) const {
